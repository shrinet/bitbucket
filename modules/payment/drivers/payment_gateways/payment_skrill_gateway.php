...
class Payment_Skrill_Gateway extends Payment_Type_Base
{
	/**
	 ...
	 * like Skrill Standard. Otherwise override build_payment_form method to build admin payment
	 * forms.
	...
	public function build_config_ui($host, $context = null)
	{
		...
		$host->add_field('business_email', 'Business Email')->tab('Configuration')->display_as(frm_text)->comment('Enter yours business account email address.', 'above')->validation()->fn('trim')->required('Please provide Skrill business account email address.')->email('Please provide valid email address in Business Email field.');                

		if ($context !== 'preview')
		{
			$host->add_form_partial($host->get_partial_path('hint.htm'))->tab('Configuration');
			$host->add_field('secret_word', 'Secret Word')->tab('Configuration')->display_as(frm_text)->comment('Enter your Secret Word as mentioned above.', 'above')->validation()->fn('trim')->required('Please provide your Skrill Secret Word.');         
		}
	...
	public function get_hidden_fields($host, $invoice, $admin = false)
	{
		...
		// $result['logo_url'] = "gateway/skrill/monlogoenvrai"; 

		$result['merchant_fields'] = "field1";      
		$result['field1'] = $invoice->id;

		$result['status_url'] = Phpr::$request->get_root_url().root_url('api_pay_skrill_status_url/'.$invoice->hash);
		if (!$admin)
		{
			$result['return_url'] = Phpr::$request->get_root_url().root_url('api_pay_skrill_return_url/'.$invoice->hash);

			$cancel_page = Cms_Page::create()->find($host->cancel_page);
			if ($cancel_page)
			{
				$result['cancel_url'] = Phpr::$request->get_root_url().root_url($cancel_page->url);
		...
		{
			$result['return_url'] = Phpr::$request->get_root_url().root_url('api_pay_skrill_return_url/'.$invoice->hash.'/admin');
			$result['cancel_url'] = Phpr::$request->get_root_url().url('payment/pay/'.$invoice->id.'?'.uniqid());
		}
	 ...
	 * Skrill needs a landing page for the auto-return feature.
	...
	{
		return array(
			'api_pay_skrill_return_url'=>'process_skrill_return_url',
			'api_pay_skrill_status_url'=>'process_skrill_status_url'
		);
	}

	public function process_skrill_status_url($params)
	{
		try
		{
			$invoice = null;

			// Find invoice and load Skrill settings
			//
			...
			if ($invoice->payment_type->class_name != 'Payment_Skrill_Gateway')
			...
			if (strtoupper(md5($field_string)) == $_POST['md5sig']
				&& $_POST['status'] == 2
				&& $_POST['pay_to_email'] == $invoice->payment_type->business_email)
			{
				// Valid transaction
				if ($invoice->mark_as_payment_processed())
				{
					$this->log_payment_attempt($invoice, 'Successful payment', 1, array(), $_POST, $field_string);
			...
			else
			{
				...
				$this->log_payment_attempt($invoice, 'Invalid payment notification', 0, array(), $_POST, $field_string);
			}
		}
		catch (Exception $ex)
		{
	...
	public function process_skrill_return_url($params)
	{
		try
		{
			// Internals
			...
			if ($return_page)
				...
				throw new Phpr_ApplicationException('Skrill Receipt page is not found');
	...
	public function page_deletion_check($host, $page)
	{
		if ($host->cancel_page == $page->id)
			throw new Phpr_ApplicationException('Page cannot be deleted because it is used in Skrill payment method as a cancel page');
	}
	...
	public function status_deletion_check($host, $status)
	{
		if ($host->invoice_status == $status->id)
			throw new Phpr_ApplicationException('Status cannot be deleted because it is used in Skrill payment method');
	}
